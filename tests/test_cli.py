...
from claude_wrapper.core.session import Message, Session
...
def create_test_session(session_id: str = "test-session", **kwargs):
    ...
    return Session(id=session_id, created_at=now, updated_at=now, **kwargs)
...
class TestCLI:
    ...
    def test_chat_with_session(self, cli_runner):
        ...
        with patch("claude_wrapper.cli.main.get_client") as mock_get_client:
            ...
            mock_get_client.return_value.session_manager.get_session = AsyncMock(
                return_value=mock_session
            )
            mock_get_client.return_value.chat = AsyncMock()
            mock_get_client.return_value.chat.return_value = ("Response", mock_session)

            result = cli_runner.invoke(app, ["chat", "Continue", "--session", "existing-session"])

            assert result.exit_code == 0
            mock_get_client.return_value.session_manager.get_session.assert_called_once_with(
                "existing-session"
            )
    ...
    def test_cleanup_command(self, cli_runner):
        ...
        with patch("claude_wrapper.cli.main.get_client") as mock_get_client:
            mock_get_client.return_value.session_manager.cleanup_old_sessions = AsyncMock(
                return_value=5
            )
    ...
    def test_delete_session(self, cli_runner):
        ...
        with patch("claude_wrapper.cli.main.get_client") as mock_get_client:
            mock_get_client.return_value.session_manager.delete_session = AsyncMock(
                return_value=True
            )
    ...
    def test_config_show(self, cli_runner):
        ...
        with patch("claude_wrapper.cli.main.config") as mock_config:
            mock_config.to_yaml.return_value = (
    ...
    def test_config_set(self, cli_runner):
        ...
        with patch("claude_wrapper.cli.main.config") as mock_config:
            result = cli_runner.invoke(
                app, ["configure", "--claude-path", "/custom/claude", "--timeout", "60"]
            )
    ...
    def test_server_command(self, cli_runner):
        ...
        with patch("uvicorn.run") as mock_uvicorn:
            result = cli_runner.invoke(app, ["server", "--host", "0.0.0.0", "--port", "8080"])
