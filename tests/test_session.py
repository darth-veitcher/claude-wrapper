...
from claude_wrapper.core.session import Message, Session, SessionManager
...
class TestSessionManager:
    ...
    async def test_list_sessions(self, session_manager):
        ...
        sessions = []
        for i in range(3):
            session = await session_manager.create_session(metadata={"index": i})
    ...
    async def test_concurrent_session_access(self, session_manager):
        ...
        async def create_and_update_session(index):
            session = await session_manager.create_session(metadata={"index": index})
            # Add some messages
    ...
    async def test_session_metadata_search(self, session_manager):
        ...
        project_a_sessions = [s for s in sessions if s.metadata.get("project") == "A"]
